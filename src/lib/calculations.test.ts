...
describe('calculations', () => {
  describe('calculateTargetWeight', () => {
    it('calculates and rounds correctly for kg (nearest 2.5kg)', () => {
      ...
      expect(calculateTargetWeight(100, 73, 'kg')).toBe(72.5);
      // 100 * 0.74 = 74. Should round to 75
      expect(calculateTargetWeight(100, 74, 'kg')).toBe(75);
    });

    it('calculates and rounds correctly for lb (nearest 5lb)', () => {
      // 200 * 0.73 = 146. Should round to 145
      expect(calculateTargetWeight(200, 73, 'lb')).toBe(145);
      // 200 * 0.74 = 148. Should round to 150
      expect(calculateTargetWeight(200, 74, 'lb')).toBe(150);
    });
  });
  ...
  describe('estimateOneRepMax', () => {
    it('estimates 1RM using Epley formula', () => {
      ...
      expect(Math.abs(result - 116.666) < 0.01).toBe(true);
    });
  ...
  describe('convertUnit', () => {
    it('converts kg to lb', () => {
      ...
      expect(Math.abs(result - 220.462) < 0.01).toBe(true);
    });

    it('converts lb to kg', () => {
      const result = convertUnit(220.462, 'lb', 'kg');
      expect(Math.abs(result - 100) < 0.01).toBe(true);
    });

    it('returns same value if units are the same', () => {
      expect(convertUnit(100, 'kg', 'kg')).toBe(100);
      expect(convertUnit(100, 'lb', 'lb')).toBe(100);
    });
  });
